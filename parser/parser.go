...
func New(l *lexer.Lexer) *Parser {
	...
	// 2つのトークンを読み込む。これによりcurToken, peekTokenのどちらも設定される
	p.nextToken()
	p.nextToken()

	// 前置演算子, 識別子, 整数リテラル用のパース関数
...
func (p *Parser) parseExpression(precedence int) ast.Expression {
	...
	// precedence: 右結合力, peekPrecedence: 左結合力
	// 結合力=トークン,演算子,オペランドが、現在の右/左のトークンと結合する強さ
	// 例えば左結合力が強いのは、次の式など: a + b * c
